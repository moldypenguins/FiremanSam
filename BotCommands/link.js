...
export default {
  data: new SlashCommandBuilder()
    .setName("link")
    .setDescription("Link Discord User to Telegram.")
    .setDMPermission(false)
    .setDefaultMemberPermissions(PermissionFlagsBits.SendMessages)
    .addSubcommand((subcommand) =>
      subcommand
        .setName("user")
        .setDescription("Link User")
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("me").setDescription("Link Self")
    ),
  ...
  async execute(client, interaction) {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      let _a = interaction.customId.split("_")[1];
      let _u = null;
      if (_a == "user") {
        _u = interaction.guild.members.cache.get(
          interaction.customId.split("_")[2]
        );
      } else {
        _u = interaction.member;
      }
