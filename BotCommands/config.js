...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName("guild")
        .setDescription("Configure guild"))
    .addSubcommand(subcommand =>
      subcommand
        .setName("factions")
        .setDescription("Configure factions"))
    .addSubcommand(subcommand =>
      subcommand
        .setName("alliance")
        .setDescription("Configure alliance")),
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if(_subcommand == "guild") {
        ...
        interaction.reply({ embeds: [{
          ...
          title: _g.guild_name,
          fields: [
            { name: "", value: "**Setting:**", inline: false }
          ]
        }], ephemeral: false });
        ...
        let _embeds = [];
        let _options = [new StringSelectMenuOptionBuilder({label: "+ New", value: "new"})];
        ...
        interaction.reply({ embeds: _embeds, components: [new ActionRowBuilder().addComponents(select)], ephemeral: false });
      ...
      if(_command == "faction" && _faction && _field) {
        let result = null;
        if(_field == "member") {
          result = await Faction.updateOne({faction_id: _faction}, {faction_member_role: `${interaction.values[0]}`});
        } else if(_field == "banker") {
          result = await Faction.updateOne({faction_id: _faction}, {faction_banker_role: `${interaction.values[0]}`});
        }
      ...
      if(_command == "faction" && _action == "add") {
        ...
        const faction_type = interaction.fields.getTextInputValue("faction_type");
